...
#
# Bug reports welcome: fkauff@biologie.uni-kl.de or the Biopython issue tracker
...
IUPACData = Struct(
    ambiguous_dna_letters = simMat.dna_extended_letters,
    ambiguous_rna_letters = simMat.rna_extended_letters,
    ambiguous_dna_values = simMat.dna_ambiguity,
    ambiguous_rna_values = simMat.rna_ambiguity,
    protein_letters = simMat.amino_acid_letters,
    unambiguous_dna_letters = simMat.dna_letters,
    unambiguous_rna_letters = simMat.rna_letters,
    )
...
class Nexus(object):
    ...
    def _format(self, options):
        ...
        if 'datatype' in options:
            ...
            # TODO: Create a fresh instance of each object for self.alphabet
            if self.datatype == 'dna' or self.datatype == 'nucleotide':
                ...
                self.ambiguous_values = {'B': 'DN', 'Z': 'EQ',
    ...
    def write_nexus_data(self, filename=None, matrix=None, exclude=[], delete=[],
        ...
        with as_handle(filename, mode='w') as fh:
            if not omit_NEXUS:
                fh.write('#NEXUS\n')
            if comment:
                fh.write('[' + comment + ']\n')
